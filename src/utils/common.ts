...
export const formatCount = (times) => {
  let formatTimeCount: any = 0
  times = times ? Number(times) : 0
  switch (true) {
    case times > 100000000:
      formatTimeCount = `${(times / 100000000).toFixed(1)}亿`
      break
    case times > 100000:
      formatTimeCount = `${(times / 10000).toFixed(1)}万`
      break
    default:
      formatTimeCount = times
  }
  return formatTimeCount
}
